...
import { c as commonjsGlobal, g as getDefaultExportFromCjs, a as getAugmentedNamespace } from "../assets/_commonjsHelpers-ca272635.js";
...
var d3Cloud = function() {
  var size = [256, 256], text = cloudText, font = cloudFont, fontSize = cloudFontSize, fontStyle = cloudFontNormal, fontWeight = cloudFontNormal, rotate = cloudRotate, padding = cloudPadding, spiral = archimedeanSpiral, words = [], timeInterval = Infinity, event = dispatch("word", "end"), timer = null, random2 = Math.random, cloud2 = {}, canvas = cloudCanvas;
...
const cloud = /* @__PURE__ */ getDefaultExportFromCjs(d3Cloud);
...
class Tags {
  ...
  async registerTagsWordCloud() {
    const wordCloudElement = document.getElementById("tag-wordcloud");
    if (!wordCloudElement) {
      return;
    }
    const d3 = await __vitePreload(() => import("../assets/index-56d353aa.js"), true ? [] : void 0);
    const cloneWords = cloneDeep(wordClouds);
    var layout = cloud().size([wordCloudElement.offsetWidth, wordCloudElement.offsetHeight]).words(cloneWords).rotate(() => {
      return Math.random() * 60 - 30;
    }).fontSize(function(d) {
      return 100 - 1 / (d.size + 1) * 120;
    }).padding(5).on("end", (words) => {
      d3.select("#tag-wordcloud").append("svg").attr("width", layout.size()[0]).attr("height", layout.size()[1]).append("g").attr("transform", "translate(" + layout.size()[0] / 2 + "," + layout.size()[1] / 2 + ")").selectAll("text").data(words).enter().append("svg:a").attr("xlink:href", function(d) {
        return d.link;
      }).attr("data-pjax", "").append("text").style("font-size", function(d) {
        return d.size;
      }).style("font-family", "Impact").style("cursor", "pointer").style("font-weight", 500).style("fill", (d) => {
...
__decorateClass([
  documentFunction()
], Tags.prototype, "registerTags", 1);
__decorateClass([
  documentFunction()
], Tags.prototype, "registerTagsWordCloud", 1);
